...
//      .flatMap(logAndReturnResponse)
  ...
  def checkForMinimumLength(
    data: ValidIntsFound
  ): EitherT[F, RawData, IntsAboveMinimum] = {
    val asEither =
      data.validInts
        .partition(_.length > 1) match {
  ...
  def acceptDonations(
    data: IntsAboveMinimum
  ): EitherT[F, RawData, IntsBelowMaximum] =
    EitherT.liftF(checkForMaximumLength(data.aboveMinimum).map { replyData =>
      IntsBelowMaximum(
        invalidInts = data.invalidInts,
  ...
  def logReply(reply: IntsBelowMaximum): EitherT[F, RawData, IntsBelowMaximum] =
    EitherT.rightT(reply)
